...
void RenderProxy::begin(QWindow * w)
{
    if (w != 0x0) {
        m_window = w;
...
void RenderProxy::end()
{
    m_winproxy->shutdown();
    m_winproxy->join();
}

void RenderProxy::keyPressCallback(int key)
{
    if (key == 'p') {
...
bool RenderProxy::makeCurrent()
{
    bool success;

    // The context is created in makeCurrent so that it is created by OpenFrames' WinProxy thread
    if (m_context == 0x0) {
        if (m_window != 0x0) {
    ...
    // else Continue using the previous context
